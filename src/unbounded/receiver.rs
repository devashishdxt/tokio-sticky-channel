...
/// Receive values from the associated [`UnboundedSender`](crate::UnboundedSender).
...
impl<T> UnboundedReceiver<T> {
    ...
    /// buffer. This indicates that no further values can ever be received from this `UnboundedReceiver`. The channel is closed
    /// when all senders have been dropped, or when [`close`](UnboundedReceiver::close) is called.
    ///
    ...
    /// from this `UnboundedReceiver`. The channel is closed when all senders have been dropped, or when
    /// [`close`](UnboundedReceiver::close) is called.
    ///
    ...
    /// [`UnboundedSender`](crate::UnboundedSender).
    ///
    /// This method returns the [`Disconnected`](TryRecvError::Disconnected) error if the channel is currently empty,
    /// and there are no outstanding [`UnboundedSender`](crate::UnboundedSender).
